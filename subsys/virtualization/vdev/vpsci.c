/**
 * @file vpsci.c
 * @brief implementation of psci-0.2 functions
 */
...
static uint32_t psci_get_function_id(struct vcpu *vcpu, arch_commom_regs_t *arch_ctxt) {
    uint64_t reg_value = *find_index_reg(0, arch_ctxt);
    return reg_value & ~((uint32_t)0);
}

static void psci_system_off(struct vcpu *vcpu, arch_commom_regs_t *arch_ctxt)
{
	zvm_shutdown_guest(vcpu->vm);
}

static void psci_system_reset(struct vcpu *vcpu, arch_commom_regs_t *arch_ctxt)
{
	zvm_reboot_guest(vcpu->vm);
}

static inline void psci_set_reg(uint32_t psci_fn, struct vcpu *vcpu,
 ...
 * x0-x4: ret
*/
static int psci_0_2_call(struct vcpu *vcpu, arch_commom_regs_t *arch_ctxt)
{
